...
router.get('/:source/:symbol', async function (req, res, next) {
    try {
        ...
        const just_values = req.query.just_values || false;

        const stocks = await StocksDAO.getStock(req.params.source, req.params.symbol, startDate, finishDate, just_values);
